...
export function RegistrarMovimientos({ setState, dataSelect, accion }) {
  ...
  useEffect(() => {
    if (dataCategorias && dataCategorias.length > 0 && !categoriaItemSelect) {
      selectCategoria(dataCategorias[0]);
    }
  }, [dataCategorias]);
  ...
  function limpiarTodo() {
    setEstado(true); // reset switch
    reset(); // limpia formulario
    selectCategoria(null); // limpia categoría seleccionada
  }

  return (
    <Container
      onClick={() => {
        (limpiarTodo(), setState());
      }}
    >
      <div
      ...
      >
        ...
        <form onSubmit={handleSubmit(guardar)} className="formulario">
          <section>
            <div>
              <label>Monto:</label>
              <div>
